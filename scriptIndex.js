...
`;
  ...
  //api's url to search heroes according to the name,whatever we search for is the value and
  // accordingly fetched from the api and results are displayed on the super hero container
  let url =
    "https://superheroapi.com/api.php/100317409418683/search/" +
    searchBar.value;
  ...
  apiRequest.onreadystatechange = function () {
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (data.response === "error") {
        displaySearchedHeroesResult.innerHTML =
          '<h2 class = "loading-and-error-font"> Oops ! There is no super hero with this name, make sure that you are spelling it correctly :) </h2> ';
        return;
      }
      ...
      // results stored in data variable and iterating through results while creating div for every result and appending them to display container
      ...
      for (let hero of data.results) {
        ...
        //adding on click to every hero ,so when clicked,directed to hero's info page
        ...
        //  created an array and checked if the super hero is already added to favourites and styled the colour of the favorite button accordingly
